...
gui
  ...
  .onChange((value) => {
    console.log(value);
    document.querySelector('.seconds-needle').style.transform = `translate(0, calc(-100% + 5px)) rotate(${value}deg)`;
  });
...
class Clock {
  ...
  setHoursOnClock() {
    ...
    for (let i = 1; i < hours + 1; i++) {
      ...
      let xPoint = (radius + 20) * Math.cos((Math.PI / 6) * i - Math.PI / 2) + center.x;
      let yPoint = (radius + 20) * Math.sin((Math.PI / 6) * i - Math.PI / 2) + center.y;
  ...
  updateHours() {
    ...
    document.querySelector('.hours-needle').style.transform = `translate(0, calc(-100% + 3.25px)) rotate(${
      (hours * 360) / 12
    }deg)`;
  }
  ...
  updateMinutes() {
    ...
    document.querySelector('.minutes-needle').style.transform = `translate(0, calc(-100% + 3.25px)) rotate(${
      (minutes * 360) / 60
    }deg)`;
  }
  ...
  updateSeconds() {
    ...
    document.querySelector('.seconds-needle').style.transform = `translate(0, calc(-100% + 1.25px)) rotate(${
      (seconds * 360) / 60
    }deg)`;
  }
}
